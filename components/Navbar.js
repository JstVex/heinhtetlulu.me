...
function MobileNavItem({ href, children }) {
    return (
        <li>
            <Popover.Button as={Link} href={href} className="block py-2 hover:text-sky-300 dark:hover:text-blue-300">
                {children}
            </Popover.Button>
        </li>
    )
...
function MobileNavigation(props) {
    return (
        <Popover {...props}>
            <Popover.Button
                className="outline-none group flex items-center rounded-full bg-white/90 px-4 py-[0.65rem]  text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10 dark:hover:ring-white/20">
                Menu
    ...
    return (
        <li>
            <Link
                href={href}
                className={clsx(
                    'relative block px-3 py-[0.65rem] transition',
                    isActive
                        ? 'text-teal-500 dark:text-teal-400'
                        : 'hover:text-sky-300 dark:hover:text-blue-300'
                )}
            >
                {children}
                {isActive && (
